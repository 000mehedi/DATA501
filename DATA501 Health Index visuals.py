...
col_needed = ['Sargassum_Health_Index', 'Marine_Biodiversity_Score', 'Water_Temperature_C', 'Dissolved_Oxygen_mg_L', 'pH_Level', 'Nutrient_Level_ppm']
...
data['Health_Category'] = pd.cut(data['Sargassum_Health_Index'], bins=bins, labels=labels, include_lowest=True)


print(data['Health_Category'].value_counts())

sns.histplot(data['Sargassum_Health_Index'], bins=20, kde=True, color='blue', edgecolor='black')
...
for i, col in enumerate(col_needed):
    ...
    # Plot
    sns.histplot(col_data, bins=20, kde=True, ax=axes[i], color='blue', edgecolor='black')
...
numeric_data = data[['Sargassum_Health_Index', 'Marine_Biodiversity_Score', 'Water_Temperature_C', 
                     'Dissolved_Oxygen_mg_L', 'pH_Level', 'Nutrient_Level_ppm']]
...
sns.lineplot(x=df_line_daily.index, y=df_line_daily['Sargassum_Health_Index'], label='Sargassum Health Index')
sns.lineplot(x=df_line_daily.index, y=df_line_daily['Marine_Biodiversity_Score'], label='Biodiversity Score')
